          ...
          <div class="row">
            ...
            <div class="col s12">
              <div class="card white-text grey darken-4 padding-15">
                <span class="orbit-eyebrow">PILOT DEVELOPMENT</span><h2>Every skill. A new advantage.</h2><p>Invest your research points and shape your strengths in space.</p>

                <div class="orbit-research-summary"><div><small>Log disks</small><strong id="logdisks"><?php echo $skillTree->logdisks; ?></strong></div><div><small>Disks per point</small><strong id="requiredLogdisks"><?php echo $requiredLogdisks; ?></strong></div><div><small>Research points</small><strong id="researchPoints"><?php echo $skillTree->researchPoints; ?></strong></div><button id="exchangeLogdisks" class="btn-small grey darken-3 waves-effect waves-light" <?php echo (($skillTree->logdisks < $requiredLogdisks) || ((array_sum((array)$skillPoints) + $skillTree->researchPoints) >= array_sum(array_column($skills, 'maxLevel'))) ? 'disabled' : '');?>>Exchange disks</button></div>
                <br>
                <div class="scrollBackground orbit-skill-grid">
                <?php foreach ($skills as $key => $value) { ?>
                  <div class="skillContainer">
                    ...
                    <strong class="orbit-skill-name"><?php echo orbit_escape($value['name']); ?></strong>
                  </div>
                <?php } ?>
                </div>
                <br>
